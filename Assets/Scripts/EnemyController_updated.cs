...
public class EnemyController2 : MonoBehaviour
{
    ...
    void Investigate()
    {
        ...
        // Общий таймер расследования (идёт всегда, а не только когда стоим)
        ...
        {
            // Небольшая пауза на точке + "оглядеться"
    ...
    private void OnEnterState(EnemyState newState)
    {
        switch (newState)
        {
            ...
            case EnemyState.Investigate:
                ...
                // Если центр расследования ещё не задан (например, подозрение выросло без шума),
                // используем последнюю известную позицию или текущую позицию врага.
                if (investigateCenter == Vector3.zero)
                {
                    if (lastHeardPosition != Vector3.zero) investigateCenter = lastHeardPosition;
                    else if (lastSeenPosition != Vector3.zero) investigateCenter = lastSeenPosition;
                    else investigateCenter = transform.position;
                }
    ...
    {
        // Если центр не задан — на всякий случай
        if (investigateCenter == Vector3.zero)
            investigateCenter = transform.position;
    ...
    public void HearNoise(Vector3 noisePosition, float noiseRadius)
    {
        ...
        // Если уже в погоне — игнорируем шум (можно поменять если хочешь)
